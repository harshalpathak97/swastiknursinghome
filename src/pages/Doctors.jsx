...
import { useParams, useNavigate } from "react-router-dom"; // Saxay
import { AppContext } from '../context/AppContext';

const Doctors = () => {
  const { speciality } = useParams(); // Ka hel speciality URL-ka
  const navigate = useNavigate(); // Isticmaal navigate
      ...
      <div className="flex flex-col sm:flex-row items-start gap-5 mt-5">
        ...
        <div className="w-full grid grid-cols-auto gap-4 gap-y-6">
          {filterDoc.map((item, index) => (
            <div
              onClick={() => navigate(`/appointment/${item._id}`)} // navigate sax ah
